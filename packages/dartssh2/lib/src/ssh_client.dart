...
class SSHClient {
  ...
  Future<void> subsystem(String subsystem) async {
    ...
    final channelController = await _openSessionChannel();
    final ok = await channelController.sendSubsystem(subsystem);
    if (!ok) {
      throw SSHChannelRequestError(
        'The server denied the $subsystem subsystem request.',
      );
    }
  }
  ...
  Future<SftpClient> sftp() async {
    ...
    final channelController = await _openSessionChannel();
    final subsystemOk = await channelController.sendSubsystem('sftp');
    if (!subsystemOk) {
      throw SSHChannelRequestError(
        'The server denied the SFTP subsystem request. '
        'Make sure the SFTP subsystem is enabled on the server.',
      );
    }
