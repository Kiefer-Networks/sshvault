...
/// Bridges SSHVault's host list into iOS Core Spotlight so users can search
/// for hosts directly from the home-screen Spotlight surface and have results
/// activate SSHVault via `NSUserActivity` / `CSSearchableItemActionType`.
///
/// Channel: `de.kiefer_networks.sshvault/spotlight` (shared verbatim with the
/// macOS implementation in `macos/Runner/SpotlightIndexer.swift` so a single
/// Dart-side service can drive both platforms).
///
/// Methods:
...
///         callback in `AppDelegate.application(_:continue:restorationHandler:)`
///         can recover it.
...
final class SpotlightIndexer: NSObject {
    ...
    /// and `macos/Runner/SpotlightIndexer.swift`. Keeping the iOS and macOS
    /// channels identical means the Dart side stays a single class with a
    /// platform-OR guard rather than two parallel implementations.
    ...
    /// fallback icon on devices that don't have SSHVault's UTI registered.
    ...
    /// Singleton kept alive by `register(messenger:)`. Spotlight calls do not
    /// need a long-lived reference, but the method-channel handler does.
    ...
    private func indexItems(
        ...
        let items: [CSSearchableItem] = raw.compactMap { dict in
            ...
            // the empty query on some iOS versions.
